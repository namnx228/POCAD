...
public class svm {
	...
	/*This method returns a probability value rather than a binary output for
	 * One Class Classifiers (as proposed in Intrusion Detection in Computer Networks 
	 * by a Modular Ensemble of One-Class Classifiers (Giacinto et al. 2007))
	 * */
	
	public static void svm_predict_values_probabilities(svm_model model, svm_node[] x, double[] dec_values, int features)
	{
		if(model.param.svm_type == svm_parameter.ONE_CLASS)	
		{
			...
			/*
			 * The normalization of sum has been commented to avoid problems with
			 * possible very low values of probabilities in case of high number
			 * of feautres. If needed the normalization can be done "externally" with
			 * respect to this methord, given that only gamma and the num of features
			 * are needed.
			 */
			//sum = sum/Math.pow((2*Math.PI*(1/model.param.gamma)),features/(double)2);
			dec_values[0] = sum;
	 ...
	 * @return
	 */
	public static double svm_predict_probabilities(svm_model model, svm_node[] x, int features)
	{	
		double[] res = new double[1];
	
		if(model.param.svm_type == svm_parameter.ONE_CLASS) 
			svm_predict_values_probabilities(model, x, res, features);
